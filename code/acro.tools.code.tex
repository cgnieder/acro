    ...
    \bool_if:NTF \l__acro_format_replace_bool
      {
        \acro_property_if_set:nnTF {#1} {#2-format}
          ...
          { \tl_use:c {l__acro_format_ \acro_if_short:nTF {#2} {short} {long} _tl} }
        {#3}
      }
      {
        \tl_use:c {l__acro_format_ \acro_if_short:nTF {#2} {short} {long} _tl}
...
\keys_define:nn {acro}
  {
    format           .multichoice: ,
    format / replace .code:n     =
      \use:c {bool_set_#1:N} \l__acro_format_replace_bool ,
    format / replace .initial:n  = false ,
    format / short   .tl_set:N   = \l__acro_format_short_tl ,
    format / short   .initial:n  = ,
    format / long    .tl_set:N   = \l__acro_format_long_tl ,
    format / long    .initial:n  = ,
    format / unknown .meta:n     =
      { format / short = #1 , format / long = #1 } ,
    short-format     .meta:n     = { format / short = #1 } ,
    long-format      .meta:n     = { format / long = #1 }
  }
...
% #1: before
% #2: cite key
\cs_new_protected:Npn \acro_cite:nn #1#2
  {
    \acro_property_if_set:nnT {#2} {cite}
      {
        #1 \__acro_cite:n {#2}
        \bool_gset_true:N \g__acro_cited_bool
      }
  }
\cs_generate_variant:Nn \acro_cite:nn {ne}
