...
If \acro\ finds a file \code{acro.cfg} present it inputs it at the end of the
package.  The only thing to be aware of is that this file is input in an expl3
programming environment.  This means spaces are ignored and must be input
explicitly with \code{\textasciitilde}.  The characters \code{:} and \code{\_}
have category code~12, \ie, they are letters and can be part of macro names.
Since the file is included in the package also \code{@} has category code~12.

Details on expl3 can be found in~\cite{interface3}.  Most likely you can find
it on your system:
\begin{bash}
  texdoc interface3
\end{bash}
