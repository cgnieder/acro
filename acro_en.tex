...
\begin{options}
  ...
  %% tooltip
  \keyval{tooltip}{text}
    sets the tooltip description for an acronym.  For actually getting
    tooltips you also need an appropriate setting of the
    \option{tooltip-cmd} option.
  %% index-sort
...
\begin{options}
  \keybool{tooltip}\Default{false}
    This options loads the \pkg{pdfcomment} package and sets the command for
    creating tooltips to \cs*{pdftooltip}.
  \keyval{tooltip-cmd}\Default{\cs*{@firstoftwo}}
    This allows users using another macro for tooltips -- maybe one provided
    by another package or some own macro.  It needs to be a macro with two
    mandatory arguments, the first being the string typeset in the \acs{pdf},
    the second being the tooltip description text.
\end{options}
